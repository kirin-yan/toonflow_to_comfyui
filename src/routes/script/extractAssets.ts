...
const defaultAssetExtractionPrompt = `# 剧本资产提取

从输入剧本中提取角色、场景和重要道具。所有结果必须且只能通过 resultTool 调用一次返回。
newAssets 每项仅包含 name、desc、type、scriptIds；existingAssetRefs 每项仅包含 name、scriptIds。
type 只能是 role、scene、tool，scriptIds 只能使用输入中提供的真实剧本 ID。
不要生成 prompt 或 assetsList 字段，不要直接输出正文、Markdown、JSON 或解释。`;
...
export default router.post(
  ...
  async (req, res) => {
    ...
    await u.db("o_script").whereIn("id", scriptIds).update({ extractState: 0, errorReason: null });

    const errors: { scriptId: number; error: string }[] = [];

    // 本地模型一次最多分析两个剧本，所有批次串行执行，避免上下文过大和并发争抢显存。
    const effectiveGroupSize = Math.min(groupSize, 2);
    const scriptGroups = chunkArray(scriptIds as number[], effectiveGroupSize);
    ...
    async function persistGroupResult(result: GroupResult) {
      ...
      // 插入新资产（不在已有列表中的）
      const uniqueNewAssets = [...new Map(newAssets.map((asset) => [asset.name.trim(), { ...asset, name: asset.name.trim() }])).values()];
      const toInsert = uniqueNewAssets.filter((asset) => asset.name && !existingMap.has(asset.name));
      if (toInsert.length) {
      ...
      await knexDb.transaction(async (trx) => {
        ...
        // 关联替换和状态更新必须一起成功，避免中途失败后丢失旧关联。
        await trx("o_script").whereIn("id", batchScriptIds).update({
          extractState: 1,
          errorReason: null,
        });
      });
    ...
    function processGroup(group: number[][]) {
      ...
      const tasks = group.map((itemIds) =>
        extractionLimit(async () => {
          ...
          try {
            const resultTool = tool({
              ...
              inputSchema: z.object({
                newAssets: z
                  ...
                  .describe("新发现的资产列表（不在已有资产列表中的），包含 name、desc、type 和使用该资产的 scriptIds"),
                existingAssetRefs: z
            ...
            if (!scriptAssetExtraction || scriptAssetExtraction.includes("assetsList") || scriptAssetExtraction.includes("`prompt`")) {
              scriptAssetExtraction = defaultAssetExtractionPrompt;
            }
              ...
              messages: [
                {
                  ...
                  content:
                    ...
                    "\n\n【最高优先级输出规则】resultTool 的 inputSchema 是唯一有效输出格式。只调用一次 resultTool；使用 newAssets 和 existingAssetRefs 字段；newAssets 每项只填写 name、desc、type、scriptIds，不填写 prompt 或 assetsList；禁止直接输出 JSON、Markdown 或解释。",
                },
                {
                  role: "user",
            ...
            const allowedScriptIds = new Set(validScriptIds);
            collectedNew = collectedNew
              .map((asset) => ({ ...asset, scriptIds: [...new Set(asset.scriptIds)].filter((id) => allowedScriptIds.has(id)) }))
              .filter((asset) => asset.scriptIds.length > 0);
            collectedExisting = collectedExisting
              .map((asset) => ({ ...asset, scriptIds: [...new Set(asset.scriptIds)].filter((id) => allowedScriptIds.has(id)) }))
              .filter((asset) => asset.scriptIds.length > 0);
      ...
      void Promise.all(tasks).catch(async (err) => {
        const reason = u.error(err).message;
        console.error("[extractAssets] 后台提取失败:", reason);
        await u.db("o_script").whereIn("id", scriptIds).where("extractState", 0).update({ extractState: -1, errorReason: reason });
      });
    }
