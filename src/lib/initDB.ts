...
从输入剧本中提取角色、场景和重要道具。所有结果必须且只能通过 resultTool 调用一次返回，不要输出正文、Markdown 或 JSON。

resultTool 参数：
- newAssets：项目中不存在的新资产。每项仅包含 name、desc、type、scriptIds。
- existingAssetRefs：项目中已经存在的资产引用。每项仅包含 name、scriptIds。

字段规则：
- name：剧本中的原始名称。
- desc：30-80 字的中文视觉描述。
- type：只能是 role、scene、tool。
- scriptIds：该资产出现的剧本 ID，只能使用输入中提供的真实 ID。

不要生成 prompt 或 assetsList 字段。提示词将在后续独立步骤生成。同一资产只返回一次；已有资产必须放入 existingAssetRefs，不得作为 newAssets 重复创建。`,
        });
      },
    },
    //小说原文表
